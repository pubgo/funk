...
func (s *Server) Start() {
	...
	if s.Config.Cert != "" || s.Config.Key != "" {
		...
		if err != nil {
			logger.Errorf("Load Certificate failed.")
			return
		}

		s.tlsLn, err = tls.Listen("tcp", s.Addr, TLSConfig)
		if err != nil {
			logger.Errorf("Get tlsListener failed.")
			return
		}
		s.TLSEnable = true
	}
	if !s.TLSEnable {
		...
		if err != nil {
			logger.Errorf("Get tcpListener failed.")
			return
		}
	}
	if s.connGrpool == nil {
	...
	for {
		...
		if s.TLSEnable {
			...
			if err1 != nil {
				logger.Errorf("Disable tcp Keepalive failed.")
			}
			conn = tcpconn
		}
		tempDelay = 0
		if conn == nil {
			logger.Errorf("get conn is nil")
			continue
		}
...
func (s *Server) GetTCPListener() (*net.TCPListener, error) {
	...
	if err != nil {
		logger.Errorf("Resolve address %s failed!", s.Addr)
		return nil, err
	}
...
func CheckAndHandleTempError(err error) bool {
	if ne, ok := err.(net.Error); ok && ne.Temporary() {
		...
		logger.Infof("http: Accept error: %v; retrying in %v", err, tempDelay)
...
func (s *Server) HandleMessage(msg *WSMessage) {
	...
	defer func() {
		if r := recover(); r != nil {
			logger.Errorf("Message: %v, handler panic: %v", msg, r)
		}
		s.handlerWG.Done()
	}()
	if msg == nil {
		logger.Errorf("get msg nil")
		return
	}
	if msg.Route == "all" {
...
func (s *Server) RegisterEventHandler(path string, event EventType, handler EventHandler) {
	...
	if _, ok := s.eventHandlers[path]; ok {
		if _, ok := s.eventHandlers[path][event]; ok {
			logger.Errorf("Path %s Event %s handler already exsit.", path, event)
			return
		}
...
func (s *Server) HandleWSConn(wsconn *ServerConn) {
	...
	defer func() {
		if e := recover(); e != nil {
			logger.WithField("stacktrace", string(debug.Stack())).Errorf("recover in watch handler %v", e)
		}
	}()
	s.elock.RLock()
	...
	if ok {
		...
		if s.Config.KeepAlive {
			logger.Infof("ws conn keepAlive: %v", wsconn.ConnInfo)
			if err := linkGrPool.Submit(func() { wsconn.KeepAlive() }); err != nil {
				logger.Errorf("server KeepLive error:%v", err)
			}
		}
		wsconn.Handle()
	} else {
...
func (s *Server) Stop() {
	...
	if err := s.SendGlobalMessage(OpClose, nil); err != nil {
		logger.Infof("SendGlobalMessage Close err: %v", err)
	}
}
...
func (s *Server) handleTCPConn(conn net.Conn) {
	err := s.connGrpool.Submit(func() {
		...
		if err != nil {
			...
			logger.Infof("upgrade error: %s, url: %s", err, path)
			return
		}
