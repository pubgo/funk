...
func (m *Migrate) FindAllVersions(ctx context.Context) ([]*VersionRecord, error) {
	...
	cursor, err := m.db.Collection(m.migrationsCollection).Find(ctx, filter, opts)
	if err != nil {
		switch {
	...
	if err := cursor.All(ctx, &rec); err != nil {
		return nil, err
	}
...
func (m *Migrate) printf(msg string, args ...any) {
	logger.Info().CallerSkipFrame(1).Msgf(msg, args...)
}
